...
const signup = (req, res) => {
    ...
    db.User.findOne({ email: req.body.email }, (err, foundUser) => {
        if (err)
            return res.status(400).json({ message: "Account not found, please try again." });
        // NOTE Return error if account already exists:
        if (foundUser)
            return res.status(400).json({ message: "Email has already been registered, please try again." });
        // NOTE Generate Hash Salt:
        bcrypt.genSalt(10, (err, salt) => {
            if (err)
                return res.status(400).json({ message: "Something went wrong with 'bcrypt', please try agian." });
            bcrypt.hash(req.body.password, salt, (err, hash) => {
                if (err)
                    return res.status(400).json({ message: "Something went wrong, please try agian." });
